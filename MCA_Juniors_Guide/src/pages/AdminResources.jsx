...
import { addSubject, getSemesters } from "../services/api";
...
const AdminResources = () => {
  ...
  useEffect(() => {
    if (formData.semester) {
      const selectedSemester = semesters.find(
        (sem) => sem.semId.toString() === formData.semester
      );
      if (selectedSemester) {
  ...
  const handleFileSelect = (event) => {
    ...
    if (validFiles.length !== selectedFiles.length) {
      setStatus({
        type: "error",
        message:
          "Some files were rejected. Only PDF, DOC, DOCX, PPT, PPTX files under 10MB are allowed.",
      });
    }
  ...
  const validateForm = () => {
    if (
      ...
      setStatus({
        type: "error",
        message: "Please fill all fields and select at least one file.",
      });
      return false;
    }
    if (showAddSubject) {
      setStatus({
        type: "error",
        message: "Please complete adding the new subject first.",
      });
      return false;
    }
    return true;
  ...
  const handleAddSubject = async () => {
    ...
    try {
      ...
      await addSubject(formData.semester, {
        id: parseInt(newSubject.id),
        name: newSubject.name,
        code: newSubject.code,
      });
      ...
      const selectedSemester = updatedSemesters.find(
        (sem) => sem.semId.toString() === formData.semester
      );
      if (selectedSemester) {
        setSubjects(selectedSemester.subjects || []);
  ...
  const getSelectedSubjectCode = () => {
    const selectedSubject = subjects.find(
      (subject) => subject.id.toString() === formData.subjectCode
    );
  ...
  const handleUpload = async () => {
    ...
    console.log(
      `Preparing to upload files for Semester ${formData.semester}, Subject ${formData.subjectCode}, Type: ${formData.type}`
    );
    ...
    try {
      ...
      const uploadType =
        formData.type === "questionPaper" ? "questionPapers" : "materials";
      const uploadedFiles = [];
      ...
      for (const file of formData.files) {
        ...
        console.log(
          `Uploading file: ${file.name} for Semester ${formData.semester}, Subject ${formData.subjectCode}, Type: ${uploadType}`
        );

        const apiUrl = `${import.meta.env.VITE_API_URL}/upload/${
          formData.semester
        }/${getSelectedSubjectCode()}/${uploadType}`;
